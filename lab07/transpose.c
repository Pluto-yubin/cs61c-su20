...
{
    // YOUR CODE HERE
    for (int i = blocksize; i < n + blocksize; i += blocksize) {
        for (int j = blocksize; j < n + blocksize; j += blocksize) {
            for (int x = i - blocksize; x < i && x < n; x += 1) {
                for (int y = j - blocksize; y < j && y < n; y += 1) {
                    dst[y + x * n] = src[x + y * n];
                }
            }
        }
